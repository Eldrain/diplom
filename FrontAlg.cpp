...
public:
	...
	//Заполняет перестановку максимальными из фронта
	int alg1(Task &task) {
		int index = 0;
		task.jobs.refresh();
	...
	//Заполняет перестановку минимальными из фронта
	int alg2(Task &task) {
		int index = 0;
		task.jobs.refresh();
		for (int i = 0; i < n; i++) {
	...
	//Заполняет перестановку работами из фронта на основе разницы в продолжительностях работы процессоров
	/*int alg3(Task &task) {
	...
	//Обновляет буффер и возвращает минимальное значение разницы в нем.
